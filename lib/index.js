...
function endpointFromUrl(pathname) {
	if (!pathname.startsWith(`${RPC_ROUTE}/`)) return void 0;
...
function sendRpcResult(res, rpcId, result) {
	res.writeHead(200, { "Content-Type": "application/json" });
	res.end(JSON.stringify({
...
function apply(ctx) {
	const dispatch = async (ep, payload) => {
	...
	const handleRpc = async (req, res) => {
		try {
			...
			const endpoint = req.method === "POST" ? endpointFromUrl(new URL(req.url ?? "/", "http://dsh.internal").pathname) : void 0;
			if (endpoint === void 0) {
				res.writeHead(404);
				res.end("not found");
				return;
	...
	}), "dsh-any-background: video upload route");
}
//#endregion
